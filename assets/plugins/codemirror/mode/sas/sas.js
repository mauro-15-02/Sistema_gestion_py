...
(function(mod) {
  ...
  CodeMirror.defineMode("sas", function () {
    ...
    define('variable', '&sysbuffr &syscc &syscharwidth &syscmd &sysfecha &sysfecha9 &sysday &sysdevic &sysdmg &sysdsn &sysencoding &sysenv &syserr &syserrortext &sysfilrc &syshostname &sysindex &sysinfo &sysjobid &syslast &syslckrc &syslibrc &syslogapplname &sysmacroname &sysmenv &sysmsg &sysncpu &sysodspath &sysparm &syspbuff &sysprocessid &sysprocessname &sysprocname &sysrc &sysscp &sysscpl &sysscpl &syssite &sysstartid &sysstartname &systcpiphostname &systime &sysuserid &sysver &sysvlong &sysvlong4 &syswarningtext', ['ALL']);
    ...
    function tokenize(stream, state) {
      ...
      if (ch === '/' && stream.eat('*')) {
        ...
        return "comentario";
      } else if (state.continueComment === true) { // in comentario block
        //comentario ends at the beginning of the line
        if (ch === '*' && stream.peek() === '/') {
          stream.next();
          state.continueComment = false;
        } else if (stream.skipTo('*')) { //comentario is potentially later in line
          stream.skipTo('*');
          stream.next();
          if (stream.eat('/'))
            state.continueComment = false;
